...
- Apple show `1870433419` still uses RSS.com and exposes five episodes. Apple case `20000130526608` supplied the historical Episode 1-2 GUIDs, case `20000144016176` is the later escalation, and RSS.com confirmed existing GUIDs are immutable. The owner approved the exact first-party Apple-only overlay in `publishing/apple-feed-overlay.json`; `publishing/apple-guid-repair.json` records the approval, before-state, and rollout gates. It is not deployed or configured in Apple yet. RSS.com, Spotify, Amazon, and the master catalog remain unchanged.
...
4. Apple show `1870433419` still uses RSS.com and publicly exposes five episodes. Follow the approved Apple-only overlay lifecycle in `publishing/apple-guid-repair.json`: deploy and independently verify the derived feed, switch only the existing show's RSS URL, request one refresh, and verify seven playable public episodes. Never replace the show or mutate RSS.com.
